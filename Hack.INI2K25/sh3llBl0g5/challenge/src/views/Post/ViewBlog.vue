...
<script>
    ...
    export default {
        ...
        async mounted() {
            const docRef = doc(db, 'bXlibG9nc2xpc3Q', this.$route.params.blogid);
            await getDoc(docRef)
                .then((res) => {
            ...
            if (this.currentBlog[0].profileID != this.id) {
                this.userRef = doc(db, 'bXl1c2Vyc2xpc3Q', this.currentBlog[0].profileID);
        ...
        methods: {
            ...
            async confirmModal() {
                ...
                this.error = false;
                this.errorMessage = "";
                const docRef = doc(db, 'bXlibG9nc2xpc3Q', this.currentBlog[0].blogID);
            ...
            async showInfo(like) {
                const docRef = doc(db, 'bXl1c2Vyc2xpc3Q', like);
                this.loading = true;
                await getDoc(docRef).then((res) => {
                    this.loading = false;
                    this.error2 = true;
            ...
            getLikesAndComments() {
                ...
                for (let i = 0; i < this.currentBlog[0].likes.length; i++) {
                    const docRef = doc(db, 'bXl1c2Vyc2xpc3Q', this.currentBlog[0].likes[i]);
                ...
                for (let i = 0; i < this.currentBlog[0].comments.length; i++) {
                    const docRef = doc(db, 'bXl1c2Vyc2xpc3Q', this.currentBlog[0].comments[i].owner);
                    getDoc(docRef).then((res) => {
