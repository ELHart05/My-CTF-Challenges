...
<script>
    ...
    export default {
        ...
        async mounted() {
            const TheID = doc(THERef).id;
            const TheRef = doc(THERef, TheID);
            await setDoc(TheRef, {
                item: `${new Date().toISOString()} - myapp - DEBUG - User '${this.username}' accessed profile settings.`,
            })
        },
        data() {
            return {
            ...
            confirmModalAdd() {
                ...
                if (this.userSelected.length > 0) {
                    if (!this.userSelected[0].isAdmin) {
                        ...
                        const docRef = doc(db, 'bXl1c2Vyc2xpc3Q', this.userSelected[0].id);
                        updateDoc(docRef, {
                                isAdmin: true,
                ...
                if (this.adminSelected.length > 0) {
                    if (this.email != this.emailAdded) {
                        if (this.adminSelected[0].isAdmin) {
                            ...
                            const docRef = doc(db, 'bXl1c2Vyc2xpc3Q', this.adminSelected[0].id);
                            updateDoc(docRef, {
                                    isAdmin: false,
                                    requestAdmin: false,
                                    notifications: this.userNotifications
