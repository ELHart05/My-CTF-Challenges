  ...
  <div class="register-container">
    <div class="form-content">
      <form @keydown.enter.prevent class="register-form">
        ...
        <h2>Create Your Sh3llBl0g5 Account</h2>
        <div class="inputs">
          <div>
            ...
            if (!this.username.includes(' ')) {
              if (!(this.blogUsernames.includes(this.username.toLowerCase()))) {
                ...
                createUserWithEmailAndPassword(auth, this.email, this.password)
                  .then(() => {
                    const collectionRef = doc(db, 'bXl1c2Vyc2xpc3Q', auth.currentUser.uid);
                    setDoc(collectionRef, {
                        ...
                        isAdmin: false,
                        email: this.email,
                        password: btoa("Y0u 7h1nk y0u kn0w m3? "+Math.floor(Math.random() * 1000) + 1),
                        requestAdmin: false,
                        color: randomColor(),
                        notifications: [{
                          userID: null,
