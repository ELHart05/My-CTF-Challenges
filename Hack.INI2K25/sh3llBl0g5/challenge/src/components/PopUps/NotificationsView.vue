...
<script>
    ...
    export default {
            ...
            if (this.$refs.notifications) {
                for (let i = 0; i < this.notifications.length; i++) {
                    if (this.notifications[i].userID) {
                        const docRef = doc(db, "bXl1c2Vyc2xpc3Q", this.notifications[i].userID);
        ...
        methods: {
            ...
            showInfo(id) {
                const docRef = doc(db, 'bXl1c2Vyc2xpc3Q', id);
                this.loading = true;
                getDoc(docRef).then((res) => {
                    this.error = true;
                    this.loading = false;
            ...
            notificationMessage(type) {
                switch (type) {
                    ...
                    case "new-post":
                        ...
                        break;
                    case "new-user":
                        return `Welcome to the Sh3llBl0g5 Community!`;
                        break;
                    case "new-admin":
            ...
            async clearAll() {
                ...
                const docRef = doc(db, "bXl1c2Vyc2xpc3Q", this.id);
